export function validateName(value) {
  ...
  if (name.length > 50) return { valid: false, message: 'Name must be no more than 50 characters long' };

  const regEx = /^[A-Za-z\-\s']+$/;
  if (!regEx.test(name)) return { valid: false, message: 'Name contains invalid characters' };
...
export function validateConfirmPassword(password, confirmPassword) {
  ...
  if (!safeCompare(password, confirmPassword)) return { valid: false, message: 'Passwords do not match' };
  return { valid: true, message: null };
}
...
export function validateBirthDay(isoDate, minAge = 18) {
  if (!isoDate) return { valid: false, message: 'Please enter your birth date' };

  const age = calculateAgeFromISO(isoDate);
  if (age === null) return { valid: false, message: 'Please enter a valid date' };
